...
async function simpleSwap(swapParams: SwapParams) {
  try {
    ...
    const { accounts, evmAccount, solanaAccount, signerKey, solanaKeypair } = await loadAccounts(swapParams, 'session2');
    
    // Step 2: Check balance for the from asset
    const balanceCheckAddress = getBalanceCheckAddress(swapParams.fromAssetId, evmAccount, solanaAccount);
...
async function main() {
  try {
    ...
    await simpleSwap({
      fromAssetId: 'ds:usdc',
      toAssetId: 'solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp/token:JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN', 
