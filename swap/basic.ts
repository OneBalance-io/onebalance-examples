...
/**
 * Perform USDC to AERO swap using basic account
 */
async function swapUSDCtoAERO() {
  try {
    console.log('🚀 Starting USDC to AERO swap using basic account...\n');

    // Load signer key for basic account
    const signerKey = readOrCacheEOAKey('session');

    console.log(`Signer Address: ${signerKey.address}`);

    // Predict account address for basic account
    ...
    // Configure basic account for V3 API
    const account = {
    ...
    console.log(`Using Basic Account: ${account.accountAddress}`);
